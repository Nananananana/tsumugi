...
- Contract: ContextPackage `1-draft`. `tsumugi context --json` emits it,
  ...
  it.
- Evaluation: `docs/evaluation-corpus.md`. **70 cases, all seven trap kinds**,
  labelled evidence, a 50-case CI tier and a held-out split, generated by
  `tools/generate_cases.py` — deterministic, no model and no seed, so the same
  command produces the same fixtures. **CI calls no model.** `eval --model
